    ...
    *comp_size = 0;
    ...
    if (l2_entry & kCompressedBit) {
        *compressed = true;
        ...
        // Bits 0..(cluster_bits-8-1) are the number of additional 512-byte sectors
        // But the actual layout depends on the cluster_bits.
        // host_offset = entry bits [cluster_bits - 8 .. 61]
        // nb_sectors = entry bits [0 .. cluster_bits - 8 - 1] + 1
        uint32_t nb_csectors_bits = cluster_bits_ - 8;
        uint64_t nb_csectors_mask = (1ULL << nb_csectors_bits) - 1;
        uint32_t nb_csectors = static_cast<uint32_t>(
            (l2_entry & nb_csectors_mask) + 1);
        uint64_t host_off = (l2_entry & ~kCompressedBit) >> nb_csectors_bits;
        // Mask out the copied bit if present
        host_off &= (1ULL << (62 - nb_csectors_bits)) - 1;
