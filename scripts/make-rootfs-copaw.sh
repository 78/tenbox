...
su - $USER_NAME -c '~/.copaw/venv/bin/playwright install chromium'

echo "Installing Playwright dependencies..."
# Mount /dev/pts if not available (chroot environment may lack it)
if [ ! -d /dev/pts ] || ! mountpoint -q /dev/pts 2>/dev/null; then
    mkdir -p /dev/pts
    mount -t devpts devpts /dev/pts 2>/dev/null || true
fi
/home/$USER_NAME/.copaw/venv/bin/playwright install-deps || {
    echo "  WARNING: playwright install-deps returned non-zero, but continuing..."
}
EOF
}
...
CHROMIUM_BIN=\$(find /home/$USER_NAME/.cache/ms-playwright -name "chrome" -type f -path "*/chrome-linux*" 2>/dev/null | head -n1)
if [ -z "\$CHROMIUM_BIN" ]; then
    echo "  WARNING: Playwright Chromium not found, skipping browser config"
    exit 0
fi
CHROMIUM_DIR=\$(dirname "\$CHROMIUM_BIN")
echo "  Found Chromium at: \$CHROMIUM_BIN"

# Configure Chrome policies to disable login prompts
# Chrome for Testing uses /etc/opt/chrome path
mkdir -p /etc/opt/chrome/policies/managed
cat > /etc/opt/chrome/policies/managed/tenbox_policy.json << 'POLICY'
{
...
# Skip first run wizard (Playwright uses google-chrome-for-testing profile)
mkdir -p /home/$USER_NAME/.config/google-chrome-for-testing
touch "/home/$USER_NAME/.config/google-chrome-for-testing/First Run"
chown -R $USER_NAME:$USER_NAME /home/$USER_NAME/.config/google-chrome-for-testing

# Create wrapper script for chromium
cat > /usr/local/bin/chromium-playwright << WRAPPER
#!/bin/bash
exec "\$CHROMIUM_BIN" --no-first-run --no-default-browser-check --disable-infobars --enable-automation=false "\\\$@"
WRAPPER
chmod +x /usr/local/bin/chromium-playwright

# Create desktop file
cat > /usr/share/applications/chromium-playwright.desktop << 'APPDESKTOP'
[Desktop Entry]
Version=1.0
Name=Chromium (Playwright)
GenericName=Web Browser
Comment=Access the Internet
Exec=/usr/local/bin/chromium-playwright %U
Terminal=false
Type=Application
Icon=chromium
...
update-alternatives --install /usr/bin/x-www-browser x-www-browser /usr/local/bin/chromium-playwright 200
update-alternatives --install /usr/bin/gnome-www-browser gnome-www-browser /usr/local/bin/chromium-playwright 200

# Configure mimeapps for user
mkdir -p /home/$USER_NAME/.config
cat > /home/$USER_NAME/.config/mimeapps.list << 'MIME'
[Default Applications]
x-scheme-handler/http=chromium-playwright.desktop
x-scheme-handler/https=chromium-playwright.desktop
text/html=chromium-playwright.desktop
application/xhtml+xml=chromium-playwright.desktop
MIME
chown -R $USER_NAME:$USER_NAME /home/$USER_NAME/.config
EOF
}
...
check "chromium"          test -x /usr/local/bin/chromium-playwright
